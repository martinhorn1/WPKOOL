...
class WPCLICommands extends \WP_CLI_Command {
	...
	/**
	 * @var object Main HKOCDI class object.
	 */
	private $ocdi;
	...
	public function list_predefined() {
		if ( empty( $this->ocdi->import_files ) ) {
			WP_CLI::error( esc_html__( 'There are no predefined demo imports for currently active theme!', 'hootkit' ) );
		}

		WP_CLI::success( esc_html__( 'Here are the predefined demo imports:', 'hootkit' ) );
	...
	/**
	 * Import content/widgets/customizer settings with the HKOCDI plugin.
	 *
	 * ## OPTIONS
	 *
	 * [--content=<file>]
	...
	public function import( $args, $assoc_args ) {
		if ( ! $this->any_import_options_set( $assoc_args ) ) {
			WP_CLI::error( esc_html__( 'At least one of the possible options should be set! Check them with --help', 'hootkit' ) );
		}
	...
	/**
	 * Import the predefined demo content/widgets/customizer settings with HKOCDI.
	 *
	 * @param int $predefined_index Index of a HKOCDI predefined demo import.
	 */
	private function import_predefined( $predefined_index ) {
		if ( ! is_numeric( $predefined_index ) ) {
			WP_CLI::error( esc_html__( 'The "predefined" parameter should be a number (an index of the HKOCDI predefined demo import)!', 'hootkit' ) );
		}

		$predefined_index = absint( $predefined_index );

		if ( ! array_key_exists( $predefined_index, $this->ocdi->import_files ) ) {
			WP_CLI::warning( esc_html__( 'The supplied predefined index does not exist! Please take a look at the available predefined demo imports:', 'hootkit' ) );
		...
		WP_CLI::log( esc_html__( 'Predefined demo import started! All other parameters will be ignored!', 'hootkit' ) );

		$selected_files = $this->ocdi->import_files[ $predefined_index ];

		if ( ! empty( $selected_files['import_file_name'] ) ) {
			WP_CLI::log( sprintf( esc_html__( 'Selected predefined demo import: %s', 'hootkit' ), $selected_files['import_file_name'] ) );
		}

		WP_CLI::log( esc_html__( 'Preparing the demo import files...', 'hootkit' ) );

		$import_files =	Helpers::download_import_files( $selected_files );

		if ( empty( $import_files ) ) {
			WP_CLI::error( esc_html__( 'Demo import files could not be retrieved!', 'hootkit' ) );
		}

		WP_CLI::log( esc_html__( 'Demo import files retrieved successfully!', 'hootkit' ) );

		WP_CLI::log( esc_html__( 'Importing...', 'hootkit' ) );

		if ( ! empty( $import_files['content'] ) ) {
			$this->do_action( 'hkocdi/before_content_import_execution', $import_files, $this->ocdi->import_files, $predefined_index );

			$this->import_content( $import_files['content'] );
		}

		if ( ! empty( $import_files['widgets'] ) ) {
			$this->do_action( 'hkocdi/before_widgets_import', $import_files );
		...
		$this->do_action( 'hkocdi/after_all_import_execution', $import_files, $this->ocdi->import_files, $predefined_index );

		WP_CLI::log( esc_html__( 'Predefined import finished!', 'hootkit' ) );
	}

	/**
	 * Import the content with HKOCDI.
	 *
	...
	private function import_content( $relative_file_path ) {
		...
		if ( ! file_exists( $content_import_file_path ) ) {
			WP_CLI::warning( esc_html__( 'Content import file provided does not exist! Skipping this import!', 'hootkit' ) );
			return false;
		}

		// Change the single AJAX call duration so the whole content import will be done in one go.
		add_filter( 'hkocdi/time_for_one_ajax_call', function() {
			return 3600;
		} );

		WP_CLI::log( esc_html__( 'Importing content (this might take a while)...', 'hootkit' ) );

		Helpers::append_to_file( '', $this->ocdi->log_file_path, esc_html__( 'Importing content' , 'hootkit' ) );
		...
		if( empty( $this->ocdi->frontend_error_messages ) ) {
			WP_CLI::success( esc_html__( 'Content import finished!', 'hootkit' ) );
		}
		else {
			WP_CLI::warning( esc_html__( 'There were some issues while importing the content!', 'hootkit' ) );
	...
	/**
	 * Import the widgets with HKOCDI.
	 *
	...
	private function import_widgets( $relative_file_path ) {
		...
		if ( ! file_exists( $widgets_import_file_path ) ) {
			WP_CLI::warning( esc_html__( 'Widgets import file provided does not exist! Skipping this import!', 'hootkit' ) );
			return false;
		}

		WP_CLI::log( esc_html__( 'Importing widgets...', 'hootkit' ) );

		WidgetImporter::import( $widgets_import_file_path );

		if( empty( $this->ocdi->frontend_error_messages ) ) {
			WP_CLI::success( esc_html__( 'Widgets imported successfully!', 'hootkit' ) );
		}
		else {
			WP_CLI::warning( esc_html__( 'There were some issues while importing widgets!', 'hootkit' ) );
	...
	/**
	 * Import the customizer settings with HKOCDI.
	 *
	...
	private function import_customizer( $relative_file_path ) {
		...
		if ( ! file_exists( $customizer_import_file_path ) ) {
			WP_CLI::warning( esc_html__( 'Customizer import file provided does not exist! Skipping this import!', 'hootkit' ) );
			return false;
		}

		WP_CLI::log( esc_html__( 'Importing customizer settings...', 'hootkit' ) );

		CustomizerImporter::import( $customizer_import_file_path );

		if( empty( $this->ocdi->frontend_error_messages ) ) {
			WP_CLI::success( esc_html__( 'Customizer settings imported successfully!', 'hootkit' ) );
		}
		else {
			WP_CLI::warning( esc_html__( 'There were some issues while importing customizer settings!', 'hootkit' ) );
	...
	private function do_action( $action, $import_files = array(), $all_import_files = array(), $selected_index = null ) {
		if ( false !== has_action( $action ) ) {
			WP_CLI::log( sprintf( esc_html__( 'Executing action: %s ...', 'hootkit' ), $action ) );
