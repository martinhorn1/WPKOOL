...
class Importer {
	...
	/**
	 * The instance of the HKOCDI\Logger class.
	 *
	 * @var object
	 */
	public $logger;
	...
	public function __construct( $importer_options = array(), $logger = null ) {
		...
		// Get the HKOCDI (main plugin class) instance.
	...
	public function import_content( $import_file_path ) {
		...
		set_time_limit( apply_filters( 'hkocdi/set_time_limit_for_demo_data_import', 300 ) );
		...
		if ( ! apply_filters( 'hkocdi/regenerate_thumbnails_in_content_import', true ) ) {
	...
	public function new_ajax_request_maybe( $data ) {
		...
		if ( $time > apply_filters( 'hkocdi/time_for_one_ajax_call', 25 ) ) {
			$response = array(
				'status'  => 'newAJAX',
			...
			$log_added = Helpers::append_to_file(
				__( 'New AJAX call!' , 'hootkit' ) . PHP_EOL . $message,
