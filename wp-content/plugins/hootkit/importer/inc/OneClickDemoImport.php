...
class OneClickDemoImport {
	...
	/**
	 * The instance of the HKOCDI\Importer class.
	 *
	 * @var object
	 */
	public $importer;
  ...
  private $plugin_page_setup = array();
	...
	protected function __construct() {
		...
		add_action( 'wp_ajax_ocdi_import_hkwidget_data', array( $this, 'import_hkwidget_data_ajax_callback' ) );
		add_action( 'wp_ajax_ocdi_after_import_data', array( $this, 'after_all_import_data_ajax_callback' ) );
		add_action( 'after_setup_theme', array( $this, 'setup_plugin_with_filter_data' ), 97 );
	...
	public function create_plugin_page() {
		$this->plugin_page_setup = apply_filters( 'hkocdi/plugin_page_setup', array(
			'parent_slug' => 'themes.php',
			'page_title'  => esc_html__( 'One Click Demo Install' , 'hootkit' ),
			'menu_title'  => esc_html__( 'Install Theme Content' , 'hootkit' ),
			'capability'  => 'import',
			'menu_slug'   => 'hootkit-content-install',
		) );
		...
		$this->plugin_page = add_submenu_page(
			...
			apply_filters( 'hkocdi/plugin_page_display_callback_function', array( $this, 'display_plugin_page' ) )
		);

		register_importer( $this->plugin_page_setup['menu_slug'], $this->plugin_page_setup['page_title'], $this->plugin_page_setup['menu_title'], apply_filters( 'hkocdi/plugin_page_display_callback_function', array( $this, 'display_plugin_page' ) ) );
	}
	...
	public function display_plugin_page() {
		require_once HK_OCDI_PATH . 'views/plugin-page.php';
	}
	...
	public function admin_enqueue_scripts( $hook ) {
		...
		if ( $this->plugin_page === $hook || ( 'admin.php' === $hook && $this->plugin_page_setup['menu_slug'] === esc_attr( $_GET['import'] ) ) ) {
			...
			wp_enqueue_script( 'ocdi-main-js', HK_OCDI_URL . 'assets/js/main.js' , array( 'jquery', 'jquery-ui-dialog' ), HK_OCDI_VERSION );
			...
			wp_localize_script( 'ocdi-main-js', 'ocdi',
				array(
					...
					'wp_customize_on'  => apply_filters( 'hkocdi/enable_wp_customize_save_hooks', false ),
					'import_popup'     => apply_filters( 'hkocdi/enable_grid_layout_import_popup_confirmation', true ),
					'theme_screenshot' => $theme->get_screenshot(),
					'texts'            => array(
						'missing_preview_image' => esc_html__( 'No preview image defined for this import.', 'hootkit' ),
						'dialog_title'          => esc_html__( 'Are you sure?', 'hootkit' ),
						'dialog_no'             => esc_html__( 'Cancel', 'hootkit' ),
						'dialog_yes'            => esc_html__( 'Yes, import!', 'hootkit' ),
						'selected_import_title' => esc_html__( 'Selected demo import:', 'hootkit' ),
					),
					'dialog_options' => apply_filters( 'hkocdi/confirmation_dialog_options', array() )
				)
			);

			wp_enqueue_style( 'ocdi-main-css', HK_OCDI_URL . 'assets/css/main.css', array() , HK_OCDI_VERSION );
		}
	}
	...
	public function import_demo_data_ajax_callback() {
		...
		ini_set( 'memory_limit', apply_filters( 'hkocdi/import_memory_limit', '350M' ) );
		...
		if ( ! $use_existing_importer_data ) {
			 ...
			 * Manually uploaded import files or predefined import files via filter: hkocdi/import_files
			 */
			if ( ! empty( $_FILES ) ) { // Using manual file uploads?
				...
				$this->import_files[ $this->selected_index ]['import_file_name'] = esc_html__( 'Manually uploaded files', 'hootkit' );
			}
			elseif ( ! empty( $this->import_files[ $this->selected_index ] ) ) { // Use predefined import files from wp filter: hkocdi/import_files.
				...
				if ( is_wp_error( $this->selected_import_files ) ) {
					...
					Helpers::log_error_and_send_ajax_response(
						...
						$this->log_file_path,
						esc_html__( 'Downloaded files', 'hootkit' )
					);
				}
				...
				$log_added = Helpers::append_to_file(
					sprintf(
						__( 'The import files for: %s were successfully downloaded!', 'hootkit' ),
				...
				wp_send_json( esc_html__( 'No import files specified!', 'hootkit' ) );
			}
		}
		...
		if ( ! $this->before_import_executed ) {
			...
			/**
			 * 2). Execute the actions hooked to the 'hkocdi/before_content_import_execution' action:
			 *
			 * Default actions:
			 * 1 - Before content import WP action (with priority 10).
			 */
			do_action( 'hkocdi/before_content_import_execution', $this->selected_import_files, $this->import_files, $this->selected_index );
		}
		...
		/**
		 * 4). Execute the actions hooked to the 'hkocdi/after_content_import_execution' action:
		 *
		 * Default actions:
		 ...
		 */
		if ( !isset( $_POST['hk_flag'] ) || $_POST['hk_flag'] != 'custpone' )
		do_action( 'hkocdi/after_content_import_execution', $this->selected_import_files, $this->import_files, $this->selected_index );
		...
		if ( false !== has_action( 'hkocdi/after_all_import_execution' ) ) {
	...
	public function import_hkwidget_data_ajax_callback() {
		Helpers::verify_ajax_call();
	...
	public function import_customizer_data_ajax_callback() {
		...
		if ( $this->use_existing_importer_data() ) {
			 ...
			 */
			do_action( 'hkocdi/customizer_import_execution', $this->selected_import_files );
		}

		
		$hk_flag = ( isset( $_POST['hk_flag'] ) && $_POST['hk_flag'] == 'custpone' ) ? true : false;
		if ( $hk_flag )
			wp_send_json( array( 'status' => 'hkwidgetsAJAX' ) );
		elseif ( false !== has_action( 'hkocdi/after_all_import_execution' ) )
			wp_send_json( array( 'status' => 'afterAllImportAJAX' ) );
		// // Request the after all import AJAX call.
		// if ( false !== has_action( 'hkocdi/after_all_import_execution' ) ) {
		// 	wp_send_json( array( 'status' => 'afterAllImportAJAX' ) );
	...
	public function after_all_import_data_ajax_callback() {
		...
		if ( $this->use_existing_importer_data() ) {
			 ...
			 */
			do_action( 'hkocdi/after_all_import_execution', $this->selected_import_files, $this->import_files, $this->selected_index );
		}
	...
	private function final_response() {
		...
		if ( empty( $this->frontend_error_messages ) ) {
			...
			$response['message'] .= sprintf(
				__( '%1$s%3$sThat\'s it, all done!%4$s%2$sThe demo content has been installed. Please check your site and make sure that content has installed correctly.%5$s', 'hootkit' ),
			...
			$response['message'] .= sprintf(
				__( '%1$sThe demo import has finished, but there were some import errors.%2$sMore details about the errors can be found in this %3$s%5$slog file%6$s%4$s%7$s', 'hootkit' ),
	...
	public function setup_plugin_with_filter_data() {
		...
		$this->import_files = Helpers::validate_import_file_info( apply_filters( 'hkocdi/import_files', array() ) );

		/**
		 * Register all default actions (before content import, widget, customizer import and other actions)
		 * to the 'before_content_import_execution' and the 'hkocdi/after_content_import_execution' action hook.
		 */
		$import_actions = new ImportActions();
		$import_actions->register_hooks();

		// Importer options array.
		$importer_options = apply_filters( 'hkocdi/importer_options', array(
			'fetch_attachments' => true,
		) );

		// Logger options for the logger used in the importer.
		$logger_options = apply_filters( 'hkocdi/logger_options', array(
